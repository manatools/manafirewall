...
class ManaFirewallHelpInfo(helpdata.HelpInfoBase):
  ...
  def __init__(self):
    ...
    configure_zone = _('After selecting "Zones" as the View and one of the Zones, you can configure the selected zone. There are some parameters for each zone:') +\
          ...
          _('<li><b>Protocols</b>: Here, you can add a protocol from a list of protocols supported by firewalld, or add a custom one by name. (to complete with what is the effect of this addition)</li>') +\
          _('<li><b>Source Ports</b>: this option allows you to define a rule using the "Add" button, specifying a source port number or range and a protocol selected from the list tcp, udp, sctp and dccp. The list view displays the already defined rules. (to complete with what is the effect of this addition)</li>') +\
          _('<li><b>Masquerading</b>: this option allows you to enable or disable masquerading for the zone (IPv4 only). (to complete with what is the effect of this addition)</li>') +\
