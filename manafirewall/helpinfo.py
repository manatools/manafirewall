...
class ManaFirewallHelpInfo(helpdata.HelpInfoBase):
  ...
  def __init__(self):
    ...
    zones = _('A zone is a set of services that can be applied to a binding. A collection of zones is predefined for different levels of trust. If you select the "Permanent" configuration, you can also add, edit or delete your own zone and define which services will be allowed in your zone. ') +\
          _("For a zone, available fields are name, version, short as more descriptive name, and description. A check box allows to active default target or to select the default behaviour for packets that don't match any rule. Targets are DROP, REJECT or ACCEPT. ") +\
          _("Predefined zones are: block - dmz - drop - external - home - home - internal - public - trusted - work")
    configure_zone = _('After selecting "Zones" for view and one of the Zones, you can configure said zone. There is some parameters for each zone:') +\
          _('<ul><li><b>Services</b>: when selected in "Configure" option, you see a list of services that are predefined. According to the zone, some services are already selected. If you tick or untick a checkbox in regard to a service, the according rules will applying to allow said service on interafces having the zone. In permanent mode, the configuration is registered directly, but not applied. It will be applied after a firewalld reload.</li>') +\
          ...
          _('<li><b>Port Forwarding</b>:  port forwarding is an application of network address translation (NAT) that redirects a communication request from one address and port number combination to another while the packets are traversing the firewall in IPv4 technology. Adding a rule needs to specify a port or port range of incoming requests and its protocol (source) and either a port or range for the destination for local forwarding or address of the destination and optionnaly port or range if port is changed.</li>') +\
    ...
    services = _('A service is a collection of rules that are applied together in relation to an application. This concept helps to apply what is need for an application by just ticking a checkbox. A lot of services are already defined, but you can add your own for an application which is not provided.') +\
               _('In view "Services", next combobox allow to choice a service. Then "Configure" Selecting one update the table with a list of couples Port-Protocol ')
